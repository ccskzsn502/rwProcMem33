...
static ssize_t OnCmdReadProcessMemory(struct ioctl_request *hdr, char __user* buf) {
	...
	while (read_size < size) {
		...
		phy_addr = va_to_pa_mm(mm, proc_virt_addr + read_size, &pte);
		if (phy_addr == 0)
			break;
		...
		got = read_ram_physical_addr_bounce(false, phy_addr, (char *)(buf + read_size), pfn_sz, bounce);
...
static ssize_t OnCmdWriteProcessMemory(struct ioctl_request *hdr, char __user* buf) {
	...
	while (write_size < size) {
		...
		phy_addr = va_to_pa_mm(mm, proc_virt_addr + write_size, &pte);
		if (phy_addr == 0)
			break;
		...
		got = write_ram_physical_addr(phy_addr, (char *)((size_t)buf + write_size), false, pfn_sz);
