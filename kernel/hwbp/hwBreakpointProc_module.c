...
{
	/* Basic user VA filter for arm64 Android (TTBR0). */
	if (addr < 0x1000ULL) {
		return false;
	}
...
/*
 * Capture a shallow AAPCS64 frame-pointer chain.
 * Layout: [fp+0]=prev_fp, [fp+8]=saved_lr. Not DWARF-complete, but CE-usable.
 */
...
static void record_hit_details(struct HWBP_HANDLE_INFO *info, struct pt_regs *regs) {
	...
	/* Always keep instruction PC in regs_info.pc */
	...
	/*
	 * CE-style hit address:
	 *  - Execute BP: hit_addr = PC
	 *  - Data BP (R/W/RW): hit_addr = FAR_EL1 (exact access) if valid, else watched bp_addr
	 * Never pretend the data-watch target equals PC.
	 */
...
static void hwbp_handler(struct perf_event *bp,
	...
	/* Exception/atomic context: never sleep, never call modify_user_hw_breakpoint.
		 * Hot path: no printk — hit rate can be extremely high. */
...
/* Core init: no sole module entry / sole proc ownership when merged. */
int hwbp_subsystem_init(void) {
	...
	printk(KERN_EMERG "hwbp: subsystem ready\n");
	return 0;
#endif
}
...
void hwbp_subsystem_exit(void) {
	...
	printk(KERN_EMERG "hwbp: subsystem exit\n");
#endif
}
