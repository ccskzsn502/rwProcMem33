...
/*
 * Runtime CFI bypass (lsnbm-style): empty-CRC build + init-time
 * patch of kernel __cfi_slowpath / __cfi_slowpath_diag to RET.
 * No post-link Python step when module is built with -fno-sanitize=cfi.
 */
...
static bool rw_bypass_cfi(void)
{
	...
	if (!patch_text) {
		printk(KERN_EMERG "hwbp: aarch64_insn_patch_text not found\n");
		return false;
	}
	...
	if (!cfi_addr) {
		...
		printk(KERN_EMERG "hwbp: no CFI slowpath symbol, skip patch\n");
		done = true;
		return true;
	}
	...
	if (rc != 0) {
		printk(KERN_EMERG "hwbp: CFI slowpath patch failed rc=%d\n", rc);
		return false;
	}

	printk(KERN_EMERG "hwbp: CFI slowpath patched @0x%lx\n", cfi_addr);
	done = true;
	return true;
}
