...
/*
 * Quiet VA->PA walk using an already-held mm (no get_task_mm/mmput, no printk).
 * Caller must hold mmap lock for this mm.
 */
static inline size_t search_va_to_pa(struct mm_struct *mm, size_t virt_addr, pte_t **out_pte)
{
	pgd_t *pgd;
	p4d_t *p4d;
	pud_t *pud;
		...
		/* Align start */
...

				{
					...
					for (addr = vstart; addr < vend && !truncated; ) {
						...
						got = read_ram_physical_addr(true, phy_addr, (char *)page_buf, chunk);
						if (got == 0) {
							tail_len = 0;
							addr = (addr + PAGE_SIZE) & ~(PAGE_SIZE - 1);
							continue;
